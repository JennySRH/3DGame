...
public class FirstSceneController : MonoBehaviour, ISceneController
{
    ...
    public void ThrowDisk()
    {
        if(diskQueue.Count > 0)
        {
            ...
            disk.SetActive(true);
            disk.AddComponent<CCAction>();
            disk.GetComponent<CCAction>().Move(disk.GetComponent<DiskData>().direction, disk.GetComponent<DiskData>().speed);
            disk.AddComponent<HitManager>();
            disk.AddComponent<PhysicalManager>().Move(1.0f);
